...
<div class="tab-content sc-admin-hidden" id="default-settings-tab">
	...
	<div>
		...
		<?php
			...
			$sc_options->description( sprintf(
				__( 'Specify a currency by using it\'s <a href="%1$s" target="_blank">3-letter %2$s code</a>. Defaults to %3$s.', 'stripe' ),
	...
	<div>
		...
		<span><?php _e( 'Verifies the zip/postal code of the card.', 'stripe' ); ?></span>
		<?php $sc_options->description( __( 'You should also enable the option to decline charges that fail zip/postal code verification in your Stripe.com account.', 'stripe' ) ); ?>
	</div>
	...
	<div>
		...
		<?php
			...
			$sc_options->description( sprintf(
				__( '%1$s is used by default to select a language based on the user\'s browser configuration. '.
				    'To select a particular language, pass the 2-letter %2$s country code, such as %3$s for Chinese. ' .
				    '<a href="%4$s" target="_blank">See list of languages supported by Stripe Checkout.</a>', 'stripe' ),
				'"auto"',
				'ISO',
				'"zh"',
